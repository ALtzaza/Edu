...
router.get("/:lessonId", authenticateJWT, isEnrolled, async (req, res) => {
  try {
    const { lessonId } = req.params; // 💡 (เพิ่ม) ตรวจสอบว่า lessonId เป็น ObjectId ที่ถูกต้องหรือไม่

    if (!mongoose.Types.ObjectId.isValid(lessonId)) {
      return res
        .status(400)
        .json({ success: false, message: "Lesson ID ไม่ถูกต้อง" });
    }
    ...
    console.error("Error fetching lesson:", err); // เพิ่ม log ที่นี่
    res
      .status(500)
      .json({
        success: false,
